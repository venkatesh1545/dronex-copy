...
-- Drop conflicting policies if they exist to avoid recursion/multiple matches
DO $$
BEGIN
  IF EXISTS (
...
FOR SELECT
USING (
  -- group owner or the member themselves can see rows
  EXISTS (
    SELECT 1 FROM public.group_chats g
    WHERE g.id = group_id AND g.owner_id = auth.uid()
  )
...
-- Optional: update-on-change triggers for updated_at
-- Ensure helper function exists (already present in project). If not, create it idempotently
DO $$
BEGIN
...
-- Attach triggers (drop if already exist)
